...
<script type="text/javascript">
	//添加
	function add() {
		window.location.href = "adddirectrecord.do";
	}

	//删除
	function doDel(id) {
		if (window.confirm("确定删除此记录吗?")) {
			window.location.href = "deletedirectrecord.do?id=" + id;
		}
	}
</script>
</head>
<body>
	...
	<div style="margin-top:10px;">
		&nbsp;&nbsp;&nbsp;
		<input type="button" onclick="javascript:add();" value="上传指导记录" />
	</div>
	...
	<table border="1" width="1000" class="sjjx-table" cellspacing="0" cellpadding="0">
		...
		<c:forEach var="d" items="${directRecordList}" varStatus="stauts">
			<tr>
				...
				<td><a href="editdirectrecord.do?id=${d.id}">${d.title}</a></td>
