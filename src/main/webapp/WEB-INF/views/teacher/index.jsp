...
<script type="text/javascript">
			...
			sub_option : {
				listeners : {
					...
					click : function(s, e, m) {
						 /* alert(s.get('name') );
						alert(s.get('text') );  */
						window.open("<%=path%>/teacher/showStateStudents.do?stateCode="+ s.get('text')+ "&year="+ year);
							}
						}
					}
				}).draw();
	...
	function aler() {
		window.location.href = "checkPracticeList.do";
	}
</script>
<style type="text/css">
#customMenuDiv {
...
<h2><font color=red>${warnPassword}</font></h2>
	...
	<div id="customMenuDiv" class="customMenuDiv">
		<span id="getCustomMenu" class="getCustomMenu"><img
			src="<%=path%>/images/addHead.png" /> </span>
		<div id="customMenuDivAjax" class="customMenuDivAjax"></div>
	</div>

	<div id="menuByUser" class="menuByUser" style="width: 53%;">
		<div id="menuByUserClose">
			<span class="menuByUserClose">保存</span> <span id="menuByUserClose2"
				class="menuByUserClose2">退出</span>
		</div>
		<div id="menuByUserAjax" class="menuByUserAjax"></div>
	</div>
	 <div id="menuByHelp" class="menuByHelp" style="width: 278px; height: 63px">
	<div id="menuByUserClose"><span class="menuByUserClose">待办事项</span></div>
	<input type="hidden" id="li" value="${li}"/>
	<input type="button" value="您有未审核的实习申请" onclick="aler()" disabled="disabled"  id="sb" style="width: 275px; ">
	</div>
</body>
</html>
