...
<body>
	...
	<div id="section_container">
		<section id="aside_section" data-role="section" class="active"
			...
			<article data-role="article" id="main_article" class="active"
				...
				<div class="scroller">
					<input value="${grade}" id="year" name="date" style="width: 80px"
						class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy'})">


					<select style="width: 100%" name="code" id="sss">
						<option value="1">请选择实习状态</option>
						<c:forEach items="${mapState}" var="mymap">
							<option name="sss" id="mymapKey" value="${mymap.key}">${mymap.value}</option>
						</c:forEach>
					</select> <input type="button" id="btn" value="查询" onclick="ajaxToChart()">
	...
	<script type="text/javascript">
					...
					.plugin(new iChart.Custom(
							{
								drawFn : function() {
									...
									var coo = chart.getCoordinate(), x = coo
											.get('originx'), y = coo
											.get('originy'), H = coo.height;
									/**
									 *在左侧的位置，设置逆时针90度的旋转，渲染文字。
									 */
									chart.target.textAlign('center')
											.textBaseline('middle').textFont(
													'600 13px Verdana')
											.fillText('', x - 40, y + H / 2,
													false, '#6d869f', false,
													false, false, -90);
		...
		function ajaxToChart() {
			var year = $("#year").val();
			var code = $("#mymapKey").val();//向后台传两个数据
			window.location.href = "index.do?date=" + year + "&code=" + code;
		}
	</script>
</body>
</html>
