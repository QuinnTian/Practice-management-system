...
var data = eval("(" + "${cm}" + ")");
	...
	function ajaxToChart() {
		...
		var code=$("#mymapKey").val();//向后台传两个数据
		 ...
		 window.location.href="getCollegeStudentCityStatistics.do?year="+year+"&code="+code;	
	} 
		...
		<section id="aside_section" data-role="section" class="active"
			...
			<input value="${grade}" id="year" name="date" style="width: 80px" class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy'})">
							 ...
							 <select style="width: 100%" name="code"  id="sss">
								<option value="1">请选择当前年份的分布地区</option>
							<c:forEach items="${map}" var="mymap">
									<option   name="sss"  id="mymapKey"  value="${mymap.key}">${mymap.key}</option>
								</c:forEach>
						</select> 
