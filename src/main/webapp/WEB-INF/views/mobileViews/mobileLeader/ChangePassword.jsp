...
<script type="text/javascript">
	...
	function up() {
		if (check_state.toString() == "true"
			...
			alert("密码修改成功");
			$("#form1").attr("action", "doPasswordEdit.do");
			$("#form1").submit();
		} else {
			alert("出错啦！！");
		}
			...
			<article data-role="article" id="main_article" data-scroll="verticle"
				...
				<div class="scroller" style="text-align:center">
					...
					<form id="form1" class="form-square" action="" method="post">
						<input type="hidden" name="tea_id" value="${teacher.id}">

						<label class="label-left hint"></label> <label id="error"
							class="label-right error"></label> <label class="label-left">教工号</label>
						<label class="label-right"> <output id="email" type="text"
								readonly="true">${teacher.login_name}</output>
						...
						<button class="submit width-full" onclick="up()">提交</button>
