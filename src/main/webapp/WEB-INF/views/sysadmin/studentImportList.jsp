...
<script type="text/javascript">
	...
	function add_one() {
		...
		if (class_id == "请选择班级") {
			...
			window.location.href = "addStudent.do?class_id=" + class_id;
		}
	}
	...
	function changeCK_grade() {
		$.ajax({
			type : "get",
			/* contentType : "application/json", */
			url : "gradeList.do?",
			data : getCanShu_grade(), //设置发送参数，即使参数为空，也需要设置                
			dataType : "text", //返回的类型为json
			success : function(data) { //成功时执行的方法					
				showPractice_grade(data);
			},
			error : function(result, status) { //出错时会执行这里的回调函数                     
				if (status == 'error') {
					alert("错了");
	...
	function getCanShu_grade() {
		var xueyuan = $("#xueyuan").val();
		var dataSend = "tiaojian=" + xueyuan;
		return dataSend;
	}
	...
	function showPractice_grade(ajaxData) {//根据返回数据显示搜索结果
		$("#xibu").html(ajaxData);
	};
	function studList() {
...
<body>
	...
	<select id="xueyuan" name="xueyuan" onChange="changeCK_grade()"
		style="width:100px">
	...
	<select id="xibu" name="xibu" onChange="changeCK()" style="width:100px">
		...
		<c:forEach var="o" items="${o_xibu}" varStatus="stauts">
			<option value="${o.id}"
				<c:if test="${o.id==dept_id1}">selected</c:if>>${o.org_name}</option>
		</c:forEach>
	</select>&nbsp;&nbsp;&nbsp;&nbsp; 所在班级
	<select id="classId" name="classId" style="width:150px">
		...
		<c:forEach var="o" items="${o_class}" varStatus="stauts">
			<option value="${o.id}"
				<c:if test="${o.id==dept_id1}">selected</c:if>>${o.org_name}</option>
		</c:forEach>
	</select>
	...
	<input type="button" onclick="javascript:add_one();" value="新增学生" />
	<!--
	李秋林
		 -->
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
