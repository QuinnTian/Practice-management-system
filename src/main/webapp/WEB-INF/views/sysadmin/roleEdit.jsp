...
<body>
	...
	<form name="TemplateRoleEditform" method="post" action="doEditRole.do">
		...
		<table border="0" width="1200">
			...
			<tr>
				...
				<td width="700"><input type="text" name="role_name"
					value="${role.role_name}" style="width:700px">
				</td>
			</tr>
			<tr>
				<td width="100">角色描述：</td>
				<td width="700"><input type="text" name="role_desc"
					value="${role.role_desc}" style="width:700px">
				</td>
			</tr>
			<tr>
			...
			<tr valign="middle">
				...
				<td>
					<table border="1" cellpadding="1" cellspacing="1" width="100%">
						<c:forEach var="sysMenuOne" items="${sysMenuLists}" varStatus="stauts">
							<c:if test="${sysMenuOne.sm_is_top=='1'}">
								<tr id="${sysMenuOne.id}">
									<td>
										<div class="checkbox" >
										<c:if test="${sysMenuOne.ischecked=='true'}"><label> <input checked="checked" type="checkbox" name="sysMenu"
												onclick="return allSelect('<c:out value="${sysMenuOne.id}"/>',this.checked)" value="${sysMenuOne.id}" > ${sysMenuOne.sm_name}</label></c:if>
										<c:if test="${sysMenuOne.ischecked=='false'}"><label> <input  type="checkbox" name="sysMenu"
												value="${sysMenuOne.id}" onclick="return allSelect('<c:out value="${sysMenuOne.id}"/>',this.checked)"> ${sysMenuOne.sm_name}</label></c:if>
										</div>
									</td>
