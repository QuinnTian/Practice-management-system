...
<body>
	<button id="hiddenBtn" style="background-color:#666666; display:none;"
		onclick="doSerch2()">Hidden Button</button>
	<input id="org_id1" value="${org_id1}"
		style="background-color:#666666; display:none;">
	<input id="grade1" value="${grade1}"
		style="background-color:#666666; display:none;">
	<h1>分组管理</h1>
	条件查询: 入学年份:
	<select id="grade">
	...
	<table border="1" id="stuTab" class="sjjx-table" cellspacing="0"
		cellpadding="0">
		<tr id="biaotou">
		...
		<c:forEach var="g" items="${result}" varStatus="stauts">
			<tr>
				<td><a href="editGroups.do?id=${g.id}">${g.group_name}</a></td>
				<td>${g.description}</td>
				<td><c:set var="practice_id" value="${g.practice_id}"
						scope="request"></c:set> <%
 	String practice_id = (String) request
 				.getAttribute("practice_id");
 ...
 %>
				...
				<td><input type="button" value="修改成员"
					onclick="updateGroupMembers('${g.id}');">
				</td>
				<td><input type="button" value="删除" onclick="doDel('${g.id}');">
				</td>
				<td><input type="button" value="导入组员"
					onclick="importStuIds('${g.id}');"></td>
			</tr>
		</c:forEach>
	</table>
