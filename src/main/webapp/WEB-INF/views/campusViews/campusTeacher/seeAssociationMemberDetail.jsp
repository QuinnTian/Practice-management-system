...
<body>
	...
	<input id="addNumber" name="addNumber" type="button" value="添加成员"
		onclick="location.href='associationMemberAdd.do?id=${id}'" />
	<input id="importNumber" name="importNumber" type="button" value="导入成员"
		onclick="location.href='importAssociationMember.do?id=${id}'" />
	<br />
	<br />
	<table id="courseTab" border="1" width="850" class="sjjx-table" cellspacing="0" cellpadding="0">
		...
		<c:forEach var="associationMembers" items="${associationMembersList }" varStatus="status">
			<tr>
				<td width="150">
					<c:set var="sam_association_id" value="${associationMembers.sam_association_id}" scope="request"/>
					<%
				...
				<td width="150">
				<fmt:parseDate var="begin_time" value="${associationMembers.begin_time }"/>
				<fmt:formatDate value="${begin_time }" pattern="yyyy-MM-dd"/>
					
				
				</td>
				<td width="150">
				<c:if test="${!empty associationMembers.end_time}">
				<fmt:parseDate var="end_time" value="${associationMembers.end_time }"/>
				<fmt:formatDate value="${end_time }" pattern="yyyy-MM-dd"/>
				</c:if>
				<c:if test="${empty associationMembers.end_time}">暂无</c:if>
				</td>
				<td width="150">
				<c:choose>
						<c:when test="${associationMembers.sam_duty==1}">会长</c:when>
						<c:when test="${associationMembers.sam_duty==2}">副会长</c:when>
						<c:otherwise>
							会员
						</c:otherwise>
					</c:choose>	
