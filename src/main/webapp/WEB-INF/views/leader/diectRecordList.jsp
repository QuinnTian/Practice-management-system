...
<body>
	...
	<c:set var="diec_orgname" value="${diec_orgname}" scope="request"></c:set>
		<%
			String diec_orgname = (String) request.getAttribute("diec_orgname");
		%>
	选择条件：院系
	<select id="xibu" name="xibu" onchange="ajaxTeacher()">
		<option value="0">请选择</option>
		<c:forEach var="org" items="${org}">
			<option value="${org.id }" <c:if test="${org.org_name==diec_orgname}"> selected</c:if>>${org.org_name }</option>
		</c:forEach>
	</select> 
	教师
	<select id="teacher" name="teacher" onchange="ajaxTeasDieRecs()">
		<option value="0">请选择</option>
		<c:forEach var="diec_tea" items="${diec_tealist}">
			<option value="${diec_tea.id }">${diec_tea.true_name}</option>
		</c:forEach>
	</select>
	月份 <input id="month" value="${summary.startDate}" 
							name="month" class="Wdate" type="text"
	...
	<table id="table" border="1" width="1000" class="sjjx-table" cellspacing="0" cellpadding="0">
		...
		<c:forEach var="direct" items="${directList}" varStatus="stauts">
		<c:set var="practice_id" value="${direct.practice_id}" scope="request"></c:set>
			<%
			...
			<tr>
				<td><% out.println(DictionaryService.findPracticeTask(practice_id).getTask_name());%></td>
				<td><a href="editdirectrecord.do?id=${direct.id}">${direct.title}</a></td>
				<td>
					<fmt:parseDate value="${direct.direct_time}" var="direct_time" />
				...
				<td><%
	    				...
	    				for (int i = 0; i < sts.length; i++) {
	    					String stuName = DictionaryService.findStudent(sts[i]).getTrue_name();
	    					bf.append(stuName + "、");
	    				}
